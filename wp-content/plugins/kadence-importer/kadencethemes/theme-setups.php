...
require_once KADENCE_IMPORTER_PATH . 'kadencethemes/kadence.php';
...
function kadence_init_importer() {
	...
	if ( 'Virtue - Premium' == $theme ) {
		...
		add_filter( 'pt-ocdi/import_files', 'kadence_import_kadence_theme_files' );
		add_action( 'pt-ocdi/after_import', 'kadence_kadence_theme_after_import' );
	}
}
add_action( 'plugins_loaded', 'kadence_init_importer' );

/**
 * Get the importer page Styles.
 */
function kadence_importer_admin_styles() {
	?>
	<link rel='stylesheet' id='importer-css' href='<?php echo esc_attr( KADENCE_IMPORTER_URL ); ?>kadencethemes/styles.css' type='text/css' media='all'/>
	<?php
}
/**
 * Check if we are using a Kadence Theme.
 */
function kadence_themename() {
	...
	if ( isset( $child ) && ! empty( $child ) ) {
		...
		} else if ( $the_theme->get( 'template' ) == 'kadence' ) {
			return 'Kadence';
		} else {
			return 'notkadence';
		}
...
function kadence_importer_page_setup( $default_settings ) {
	...
	$default_settings['page_title']  = esc_html__( 'Kadence WP Importer', 'kadence-importer' );
...
function kadence_importer_page_title() {
	echo '<h1>Kadence WP Importer</h1>';
}
...
function kadence_importer_page_content() {
	...
	<h3><?php echo esc_html__( 'Notice' ); ?></h3>
	<h4 class="kt-subhead"><?php echo esc_html__( '*Using this importer will override all your theme options settings.', 'kadence-importer' ); ?></h4>
	<h4 class="kt-subhead"><?php echo esc_html__( '*This importer is designed for new/empty sites with no content.', 'kadence-importer' ); ?></h4>
	<h4 class="kt-subhead">
	<?php
	echo esc_html__( '*If you want to reset your site after testing out the demo content you can use this plugin: ', 'kadence-importer' );
	echo '<a href="https://wordpress.org/plugins/wp-reset/">' . esc_html__( 'WordPress Reset', 'kadence-importer' ) . '</a>';
	?>
	</h4>
	</div>
